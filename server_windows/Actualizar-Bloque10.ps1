...
Write-Host 'ELEGANCE BLOQUE 10 - DATOS, RESPALDO E INVENTARIO MOVIL' -ForegroundColor Cyan
Write-Host 'La carpeta persistente no sera eliminada.' -ForegroundColor Green
...
if(Test-Path $AppDir){
  $previous=Join-Path $UpdatesDir "app-pre-bloque10-$Stamp"
  New-Item -ItemType Directory -Force -Path $previous | Out-Null
  Copy-Item (Join-Path $AppDir '*') $previous -Recurse -Force
...
if($LASTEXITCODE -ne 0){ throw 'No se pudieron validar las dependencias.' }
...
# Recupera solo usuarios desde una base antigua dentro de app\data cuando la base persistente no tiene cuenta.
...
Unregister-ScheduledTask -TaskName 'Elegance Server Watchdog' -Confirm:$false -ErrorAction SilentlyContinue
...
Register-ScheduledTask -TaskName 'Elegance Server' -Action $action -Trigger $triggers -Principal $principal -Settings $settings -Description 'Elegance con base persistente fija en C:\EleganceServer\data.' -Force | Out-Null
Start-ScheduledTask -TaskName 'Elegance Server'

$ready=$false
for($i=0;$i -lt 120;$i++){ Start-Sleep 1; try{$h=Invoke-RestMethod "http://127.0.0.1:$Port/api/health" -TimeoutSec 3;if($h.status -eq 'ok'){$ready=$true;break}}catch{} }
if(-not $ready){ throw "Elegance no respondio. Revisa $LogsDir\server.log" }

Write-Host 'BLOQUE 10 INSTALADO CORRECTAMENTE' -ForegroundColor Green
Write-Host "Base persistente: $DataDir\elegance.sqlite3" -ForegroundColor Cyan
Write-Host "Diagnostico: http://127.0.0.1:$Port/database-diagnostics" -ForegroundColor Cyan
Write-Host "Inventario: http://127.0.0.1:$Port/inventory" -ForegroundColor Cyan
Write-Host "Carga movil: http://127.0.0.1:$Port/mobile-center" -ForegroundColor Cyan
Write-Host 'Reinicia Windows para comprobar el arranque automatico.' -ForegroundColor Yellow
