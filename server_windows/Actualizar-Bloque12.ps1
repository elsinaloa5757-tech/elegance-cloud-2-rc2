...
Write-Host 'ELEGANCE BLOQUE 12 - RECUPERACION INTEGRAL DE CATALOGO Y MULTIMEDIA' -ForegroundColor Cyan
...
if(Test-Path $AppDir){$previous=Join-Path $UpdatesDir "app-pre-bloque12-$Stamp";New-Item -ItemType Directory -Force -Path $previous|Out-Null;Copy-Item (Join-Path $AppDir '*') $previous -Recurse -Force;Remove-Item $AppDir -Recurse -Force}
...
try { & $Py -c "from services.legacy_recovery import recover_empty_business_data; print(recover_empty_business_data())" } finally { Pop-Location }
...
Write-Host 'BLOQUE 12 INSTALADO CORRECTAMENTE' -ForegroundColor Green
Write-Host "Configuracion inicial requerida: $($status.setupRequired)" -ForegroundColor Cyan
Write-Host "Recuperacion: http://127.0.0.1:$Port/recovery-center" -ForegroundColor Cyan
