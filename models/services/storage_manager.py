...
def _upload_rows(rows: list[sqlite3.Row]) -> dict[str, Any]:
    # The request-wide durable SQLite lease already snapshots the database.
    # Creating another full SQLite backup for every phone photo can keep the
    # Postgres advisory lock occupied until Vercel terminates the function.
    if not os.getenv("ELEGANCE_SERVERLESS", "").strip() == "1":
