...
function render(){
 ...
 <p class=muted>${esc(g.explanation||'')}</p><div class=photos>${fs.map(f=>{const o=f.outputs||{},url=o.thumbnailUrl||o.webpUrl||'';return `<div class="photo ${f.id===g.cover_file_id?'cover':''}">${url?`<img src="${esc(url)}" loading=lazy>`:'<div style="height:134px">Sin miniatura</div>'}<small>${esc(f.filename)}</small><button onclick="cover(${g.group_no},'${f.id}')">Portada</button><button class=danger onclick="delFile('${f.id}')">Quitar</button></div>`}).join('')}</div>
